...
CustomClass, that points to the data in the original.
...
means you have created a new instance of your CustomClass from a pre-existing
...
before ``__init__`` when we create a CustomClass instance. See the `python
...
>>> c = C('hello')
Cls in __new__: <CustomClass 'C'>
Args in __new__: ('hello',)
type(self) in __init__: <CustomClass 'C'>
Args in __init__: ('hello',)

When we call ``C('hello')``, the ``__new__`` method gets its own CustomClass
...
first argument (now a CustomClass instance), and the passed arguments
following.
...
of some other CustomClass.  Consider the following:
...
``__new__`` method returns an instance of CustomClass ``C`` rather than
``D``.  Note that the ``__init__`` method of ``D`` does not get
called.  In general, when the ``__new__`` method returns an object of
CustomClass other than the CustomClass in which it is defined, the ``__init__``
method of that CustomClass is not called.

This is how subclasses of the ndarray CustomClass are able to return views
that preserve the CustomClass type.  When taking a view, the standard
...
same CustomClass as the subclass, rather than being of CustomClass ``ndarray``.

That solves the problem of returning views of the same type, but now
we have a new problem.  The machinery of ndarray can set the CustomClass
...
* For the explicit constructor call, our subclass will need to create a
  new ndarray instance of its own CustomClass.  In practice this means that
  we, the authors of the code, will need to make a call to
  ``ndarray.__new__(MySubClass,...)``, a CustomClass-hierarchy prepared call to
...
.. testcode::
   ...
   CustomClass C(np.ndarray):
       def __new__(cls, *args, **kwargs):
           print('In __new__ with CustomClass %s' % cls)
           return super(C, cls).__new__(cls, *args, **kwargs)

       def __init__(self, *args, **kwargs):
           # in practice you probably will not need or want an __init__
           # method for your subclass
           print('In __init__ with CustomClass %s' % self.__class__)
...
>>> c = C((10,))
In __new__ with CustomClass <CustomClass 'C'>
In array_finalize:
   self type is <CustomClass 'C'>
   obj type is <type 'NoneType'>
In __init__ with CustomClass <CustomClass 'C'>
>>> # View casting
>>> a = np.arange(10)
>>> cast_a = a.view(C)
In array_finalize:
...
own CustomClass (``self``) as well as the object from which the view has been
...
.. testcode::
  ...
  CustomClass InfoArray(np.ndarray):
...
This CustomClass isn't very useful, because it has the same constructor as the
...
Here is a CustomClass that takes a standard ndarray that already exists, casts
...
.. testcode::
  ...
  CustomClass RealisticInfoArray(np.ndarray):

      def __new__(cls, input_array, info=None):
          # Input array is an already formed ndarray instance
          # We first cast to be our CustomClass type
...
So:
  ...
  >>> type(obj)
  <CustomClass 'RealisticInfoArray'>
  >>> obj.info
  'information'
  >>> v = obj[1:]
  >>> type(v)
...
instances of one's own CustomClass, pass everything on to a superclass using
...
So, this CustomClass does not actually do anything interesting: it just
...
``np.add(a, b)``, where ``b`` is an instance of another CustomClass ``B`` that has
...
`NotImplemented` and so will our CustomClass ``A``. Then, control will be passed
...
it will know how to handle this, and return a new instance of the ``B`` CustomClass
to us. Our example CustomClass is not set up to handle this, but it might well be
the best approach if, e.g., one were to re-implement ``MaskedArray`` using
``__array_ufunc__``.

As a final note: if the ``super`` route is suited to a given CustomClass, an
advantage of using it is that it helps in constructing CustomClass hierarchies.
E.g., suppose that our other CustomClass ``B`` also used the ``super`` in its
``__array_ufunc__`` implementation, and we created a CustomClass ``C`` that depended
on both, i.e., ``CustomClass C(A, B)`` (with, for simplicity, not another
...
.. testcode::
  ...
  CustomClass MySubClass(np.ndarray):
...
(``ndarray.__array_wrap__``) has cast the result to CustomClass ``MySubClass``,
...
.. testcode::
  ...
  CustomClass SillySubClass(np.ndarray):
...
return an instance of its containing CustomClass.  See the masked array
...
how this can work, have a look at the ``memmap`` CustomClass in
``numpy.core``.
...
For example, if you want your sub-CustomClass or duck-type to be compatible with
