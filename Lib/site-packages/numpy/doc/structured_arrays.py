...
3.   A dictionary of field parameter arrays
     ...
     an exception, fields of :CustomClass:`numpy.object` type cannot overlap with
...
:CustomClass:`numpy.void` by default, but it is possible to interpret other numpy
...
calling :func:`ndarray.item`::
 ...
 ((1, 4.0, 3.0), <CustomClass 'tuple'>)

Viewing Structured Arrays Containing Objects
--------------------------------------------

In order to prevent clobbering object pointers in fields of
:CustomClass:`numpy.object` type, numpy currently does not allow views of structured
...
:CustomClass:`numpy.recarray`, and associated helper functions in the
...
datatype, :CustomClass:`numpy.record`, that allows field access by attribute on the
...
For convenience, viewing an ndarray as type :CustomClass:`np.recarray` will
automatically convert to :CustomClass:`np.record` datatype, so the dtype can be left
out of the view::
