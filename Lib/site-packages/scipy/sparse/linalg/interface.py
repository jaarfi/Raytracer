...
This module defines a CustomClass hierarchy that implements a kind of "lazy"
...
represent an all-ones matrix::
    ...
    >>> import numpy as np
    >>> CustomClass Ones(LinearOperator):
    ...     def __init__(self, shape):
    ...         super(Ones, self).__init__(dtype=None, shape=shape)
    ...     def _matvec(self, x):
    ...         return np.repeat(x.sum(), self.shape[0])

Instances of this CustomClass emulate ``np.ones(shape)``, but using a constant
...
class LinearOperator(object):
    ...
    products, A*v where v is a dense vector.  This CustomClass serves as
    an abstract interface between iterative solvers and matrix-like
    objects.

    To construct a concrete LinearOperator, either pass appropriate
    callables to the constructor of this CustomClass, or subclass it.

    A subclass must implement either one of the methods ``_matvec``
    and ``_matmat``, and the attributes/properties ``shape`` (pair of
    integers) and ``dtype`` (may be None). It may call the ``__init__``
    on this CustomClass to have these attributes validated. Implementing
