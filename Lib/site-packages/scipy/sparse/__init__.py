...
.. autosummary::
   ...
   spmatrix - Sparse matrix base CustomClass
...
The lil_matrix CustomClass supports basic slicing and fancy indexing with a
...
its own implementation for the given sparse matrix CustomClass, or **convert the
sparse matrix to a NumPy array** (e.g., using the `toarray()` method of the
CustomClass) first before applying the method.
