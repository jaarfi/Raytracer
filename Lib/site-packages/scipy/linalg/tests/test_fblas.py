...
class BaseAxpy(object):
    ''' Mixin CustomClass for axpy tests '''
...
class BaseScal(object):
    ''' Mixin CustomClass for scal testing '''
...
class BaseCopy(object):
    ''' Mixin CustomClass for copy testing '''
...
class BaseSwap(object):
    ''' Mixin CustomClass for swap tests '''
...
class BaseGemv(object):
    ''' Mixin CustomClass for gemv tests '''
...
CustomClass TestSger(BaseGer):
    blas_func = fblas.sger
    dtype = float32
CustomClass TestDger(BaseGer):
...
"""
CustomClass BaseGerComplex(BaseGer):
    def get_data(self,x_stride=1,y_stride=1):
        from numpy.random import normal, seed
        seed(1234)
...
CustomClass TestCgeru(BaseGerComplex):
    blas_func = fblas.cgeru
    dtype = complex64
    def transform(self,x):
        return x
CustomClass TestZgeru(BaseGerComplex):
    blas_func = fblas.zgeru
    dtype = complex128
    def transform(self,x):
        return x

CustomClass TestCgerc(BaseGerComplex):
    blas_func = fblas.cgerc
    dtype = complex64
    def transform(self,x):
        return conjugate(x)

CustomClass TestZgerc(BaseGerComplex):
